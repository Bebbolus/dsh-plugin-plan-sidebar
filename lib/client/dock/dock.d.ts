...
export interface DockRootProps {
    ...
    /** Shared tool-detail event source (the tools tab emits); the tool modal consumes it. */
    tools: ToolDetailEvents;
    ...
    /** Localized tools copy for the tool detail modal (plugin passes ctx.locale.bind(TOOLS_NS)). */
    toolsT: TranslateNS<'betterSidebar.tools'>;
    /** Details-column panel actions (open/close the sidebar). */
    layout: DockLayoutActions;
}
export declare function DockRoot({ useSessions, useWorkspaces, rpc, tabs, events, tools, settings, t, toolsT, layout }: DockRootProps): JSX.Element;
