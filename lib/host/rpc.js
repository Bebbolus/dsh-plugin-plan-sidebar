...
async function dispatch(services, endpoint, payload, signal) {
    switch (endpoint) {
        ...
        case 'plan/get': {
            try {
                const planFile = path.join(process.env.WORKSPACE_DIR || '/workspace', '.dsh', 'tasks', 'plan.json');
                let planData = { plan_id: null, title: 'In attesa di piano operativo...', status: 'PENDING', tasks: [] };
                try {
