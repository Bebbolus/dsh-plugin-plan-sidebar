    ...
    switch (endpoint) {
            ...
            try {
                ...
                if (sid) {
                    ...
                    try {
                        ...
                        return toRpcResult({ ok: true, value: { plan_id: null, title: 'No active plan for this session', status: 'IDLE', tasks: [] } });
                    }
                }
                const planFile = path.join(tasksDir, 'plan.json');
                let planData = { plan_id: null, title: 'Waiting for operational plan...', status: 'IDLE', tasks: [] };
                try {
        ...
        case 'plan/clear': {
            try {
                ...
                return toRpcResult({ ok: true, value: { plan_id: null, title: 'Plan cleared', status: 'IDLE', tasks: [] } });
            } catch (e) {
                return toRpcResult({ ok: false, error: e });
            }
        }
        ...
        case 'reach/status': {
            try {
                const credsFile = path.join(process.env.WORKSPACE_DIR || '/workspace', '.dsh', 'social_credentials.json');
                let creds = {};
                try {
                    const raw = await fsPromises.readFile(credsFile, 'utf8');
                ...
                return toRpcResult({
                    ...
                    value: {
                        twitter: {
                            configured: !!(creds.twitter?.auth_token),
                            masked: creds.twitter?.auth_token ? creds.twitter.auth_token.slice(0, 4) + '••••••••' : null
                        },
                        instagram: {
                            configured: !!(creds.instagram?.session_id),
                            masked: creds.instagram?.session_id ? creds.instagram.session_id.slice(0, 4) + '••••••••' : null
                        },
                        reddit: {
                            configured: !!(creds.reddit?.session_id),
                            masked: creds.reddit?.session_id ? creds.reddit.session_id.slice(0, 4) + '••••••••' : null
                        },
                        jina: { configured: true, ready: true },
                        searxng: { configured: true, ready: true }
                    }
                });
            } catch (e) {
                return toRpcResult({ ok: false, error: e });
        ...
        case 'reach/configure': {
            try {
                ...
                const plat = (payload?.platform || '').toLowerCase();
                if (plat === 'instagram' || plat === 'ig') {
                    creds.instagram = {
                        session_id: payload.session_id || payload.cookie_string || '',
                        updated_at: new Date().toISOString()
                    };
                    process.env.INSTAGRAM_SESSION_ID = creds.instagram.session_id;
                    ...
                    process.env.TWITTER_AUTH_TOKEN = creds.twitter.auth_token;
                    process.env.TWITTER_CT0 = creds.twitter.ct0;
                }
                await fsPromises.mkdir(path.dirname(credsFile), { recursive: true });
                await fsPromises.writeFile(credsFile, JSON.stringify(creds, null, 2), 'utf8');
                return toRpcResult({ ok: true, value: { success: true, platform: plat } });
            } catch (e) {
                return toRpcResult({ ok: false, error: e });
            }
        }
